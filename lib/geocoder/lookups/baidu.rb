    ...
    def results(query, reverse = false)
      ...
      when 1, 3, 4
        raise_error(Geocoder::Error, "server error.") ||
          log("Baidu Geocoding API error: server error.")
      when 2
        raise_error(Geocoder::InvalidRequest, "invalid request.") ||
          log("Baidu Geocoding API error: invalid request.")
      when 5
        raise_error(Geocoder::InvalidApiKey, "invalid api key") ||
          log("Baidu Geocoding API error: invalid api key.")
      when 101, 102, 200..299
        raise_error(Geocoder::RequestDenied, "request denied") ||
          log("Baidu Geocoding API error: request denied.")
      when 300..399
        raise_error(Geocoder::OverQueryLimitError, "over query limit.") ||
          log("Baidu Geocoding API error: over query limit.")
      end
      return []
    end
