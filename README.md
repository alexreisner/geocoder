...
* using [`select` method (selects one or more columns)](https://api.rubyonrails.org/classes/ActiveRecord/QueryMethods.html#method-i-select)
  * See also Known Issues [using-near-with-select](#using-near-with-select) section
* using the [`pluck` method (gets an array with selecting one or more columns)](https://api.rubyonrails.org/classes/ActiveRecord/Calculations.html#method-i-pluck)
  * See also Known Issues [using-near-with-pluck](#using-near-with-pluck) section
...
#### Using `near` with `select`

You cannot reduce number of getting columns using [ActiveRecord's `select` method](https://api.rubyonrails.org/classes/ActiveRecord/QueryMethods.html#method-i-select) after `near` scope.

For example, the following code:

```ruby
Venue.near("Austin, TX", 200, min_radius: 40).select(:id, :address)
```

will produce a SQL as follows:

```sql
SELECT
    venues.*, -- unexpectedly added
    (69.09332411348201 * ABS(venues.latitude - 30.2711286) * 0.7071067811865475) + (59.836573914187355 * ABS(venues.longitude - - 97.7436995) * 0.7071067811865475) AS distance,
    CASE
        WHEN(
            venues.latitude >= 30.2711286
        AND venues.longitude >= - 97.7436995
        ) THEN 45.0
        WHEN(
            venues.latitude < 30.2711286
        AND venues.longitude >= - 97.7436995
    ...
    END AS bearing,
    "venues"."id",     -- added
    "venues"."address" -- added
FROM
    "venues"
WHERE
    (
        venues.latitude BETWEEN 27.37649293778304 AND 33.16576426221696
    AND venues.longitude BETWEEN - 101.09533124708325 AND - 94.39206775291676
    )
ORDER BY
    distance ASC
```

because `venues.*` added by `near` scope is preferred to columns you specified.
This may cause performance degradation.

Instead, you need to use `near` scope with [`select` option](https://github.com/alexreisner/geocoder/blob/b546dd637672b9bf84abf692b4594f39cdc0ad48/lib/geocoder/stores/active_record.rb#L104) in such situation.

For example, the following code:

```ruby
Venue.near("Austin, TX", 200, min_radius: 40, select: "id, name")
```
...
The above SQL has only necessary columns.

#### Using `near` with `pluck`

You cannot use [ActiveRecord's pluck method](https://api.rubyonrails.org/classes/ActiveRecord/Calculations.html#method-i-pluck) after `near` scope.

For example, the following code:

```ruby

Venue.near("Austin, TX", 200, min_radius: 40).pluck(:id)
```
...
```sql
SELECT
    "venues"."id" -- replaced from "venues.*" and more expressions generated by geocoder
FROM
    "venues"
WHERE
    (
...
This SQL will cause error like `no such column: distance (SQLite3::SQLException)`, because  columns (e.g. `distance`) added by geocoder get replaced with [ActiveRecord's pluck method](https://api.rubyonrails.org/classes/ActiveRecord/Calculations.html#method-i-pluck), and the latter takes precedence.
...
Instead, you try to combine `near` scope with [`select` option](https://github.com/alexreisner/geocoder/blob/b546dd637672b9bf84abf692b4594f39cdc0ad48/lib/geocoder/stores/active_record.rb#L104) and [Enumerable's pluck method](https://api.rubyonrails.org/classes/Enumerable.html#method-i-pluck) like as:

```ruby
Venue.near("Austin, TX", 200, min_radius: 40, select: "id").to_a.pluck(:id)
```
